...
// Force Node's own DNS resolver, bypassing whatever the OS/router is doing.
// Fixes "querySrv ECONNREFUSED _mongodb._tcp...." on networks that ignore
// the system DNS setting for SRV record lookups.
...
// One row per player+game: how many times they played, their best and
// most recent score, best streak, and when they last played. This powers
// the "who played how many times and what score" view on the homepage.
app.get('/api/summary', async (req, res) => {
  try {
    const summary = await PlaySession.aggregate([
      { $sort: { completedAt: 1 } }, // oldest -> newest, so $last below means "most recent"
      {
        $group: {
