...
const NumberChip = React.memo(function NumberChip({ value, size = 'md', tone = 'amber' }) {
  const sizeClasses =
    size === 'lg'
      ? 'min-w-[3.25rem] px-3 py-1.5 text-4xl [@media(min-width:640px)_and_(min-height:560px)]:min-w-[4rem] [@media(min-width:640px)_and_(min-height:560px)]:px-4 [@media(min-width:640px)_and_(min-height:560px)]:py-2 [@media(min-width:640px)_and_(min-height:560px)]:text-5xl'
      : 'min-w-[2.5rem] px-2 py-1 text-2xl [@media(min-width:640px)_and_(min-height:560px)]:min-w-[3rem] [@media(min-width:640px)_and_(min-height:560px)]:px-2.5 [@media(min-width:640px)_and_(min-height:560px)]:py-1.5 [@media(min-width:640px)_and_(min-height:560px)]:text-3xl';
  const toneClasses =
    tone === 'white'
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-5xl flex-col items-center overflow-hidden px-3 py-2 [@media(min-width:640px)_and_(min-height:560px)]:px-4 [@media(min-width:640px)_and_(min-height:560px)]:py-3">
        ...
        {phase === 'complete' ? (
          ...
          <div className="flex w-full flex-1 min-h-0 flex-col items-center justify-center overflow-y-auto">
            ...
            <div className="mt-2 flex min-h-[2.5rem] flex-col items-center gap-1.5 pb-2 [@media(min-width:640px)_and_(min-height:560px)]:mt-3">
              {feedback && (
                <p className="font-body animate-pop-in rounded-full bg-white/90 px-4 py-1.5 text-xs font-bold text-amber-800 shadow [@media(min-width:640px)_and_(min-height:560px)]:text-sm">
                  {feedback.msg}
                </p>
              )}
            </div>
...
export default function Game6() {
  return (
    <NameGate gameLabel="Game 6: Polly's Treasure Quest">
      <GameAccessGate gameNumber={6} gameLabel="Game 6: Polly's Treasure Quest">
        <Game6Inner />
      </GameAccessGate>
    </NameGate>
  );
...
function PirateBackdrop() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      <div className="absolute left-[8%] top-[8%] animate-float-slow text-3xl opacity-90 [@media(min-width:640px)_and_(min-height:560px)]:text-4xl">☁️</div>
      <div className="absolute right-[10%] top-[14%] animate-float-slow text-2xl opacity-80 [@media(min-width:640px)_and_(min-height:560px)]:text-3xl" style={{ animationDelay: '1.5s' }}>
        ☁️
      </div>
      <div className="absolute right-[14%] top-[26%] animate-bob-slow text-3xl opacity-90 [@media(min-width:640px)_and_(min-height:560px)]:text-4xl">⛵</div>
      <div className="absolute left-[6%] top-[36%] text-2xl opacity-80 [@media(min-width:640px)_and_(min-height:560px)]:text-3xl">🦅</div>
      <div className="absolute bottom-0 left-0 right-0 h-1/3 bg-gradient-to-t from-amber-200/80 to-transparent" />
      <div className="absolute bottom-2 left-[10%] text-4xl opacity-90 [@media(min-width:640px)_and_(min-height:560px)]:bottom-3 [@media(min-width:640px)_and_(min-height:560px)]:text-5xl">🌴</div>
      <div className="absolute bottom-2 right-[8%] text-3xl opacity-80 [@media(min-width:640px)_and_(min-height:560px)]:bottom-3 [@media(min-width:640px)_and_(min-height:560px)]:text-4xl">🏝️</div>
    </div>
  );
}
...
const StarMeter = React.memo(function StarMeter({ stars, total, dark }) {
  ...
  return (
    <div className="flex items-center gap-2" aria-label={`${stars} out of ${total} stars earned`}>
      ...
      <div className={`h-2.5 w-16 overflow-hidden rounded-full [@media(min-width:640px)_and_(min-height:560px)]:w-24 ${dark ? 'bg-slate-200' : 'bg-white/50'}`}>
        <div
      ...
      <span className={`font-body text-xs font-extrabold [@media(min-width:640px)_and_(min-height:560px)]:text-sm ${dark ? 'text-slate-700' : 'text-amber-900 drop-shadow-sm'}`}>
        {stars}/{total}
      </span>
    </div>
  );
...
function PollyPrompt({ round, streak, isWrong }) {
  return (
    <div className="mt-1 flex flex-col items-center gap-1 [@media(min-width:640px)_and_(min-height:560px)]:mt-2">
      <div className="relative">
        <span className={cn('inline-block text-4xl [@media(min-width:640px)_and_(min-height:560px)]:text-5xl', isWrong ? 'animate-shake' : 'animate-mascot-idle')}>
          🦜
        </span>
        {streak >= 2 && (
      ...
      <div className="animate-pop-in relative max-w-xs rounded-2xl bg-white px-4 py-2 text-center shadow-[0_5px_0_rgba(0,0,0,0.12)] [@media(min-width:640px)_and_(min-height:560px)]:max-w-sm">
        <span className="absolute -top-1.5 left-1/2 h-3 w-3 -translate-x-1/2 rotate-45 bg-white" />
        {isWrong ? (
          <p className="font-body text-xs font-bold text-orange-600 [@media(min-width:640px)_and_(min-height:560px)]:text-sm">Arr, not that one! 🦜</p>
        ) : round.type === 'part' ? (
          <p className="font-body text-xs font-bold text-slate-700 [@media(min-width:640px)_and_(min-height:560px)]:text-sm">
            Find the key that makes <span className="text-amber-600">{formatNumber(round.whole, round.format)}</span>!
          </p>
        ) : (
          <p className="font-body text-xs font-bold text-slate-700 [@media(min-width:640px)_and_(min-height:560px)]:text-sm">Which chest holds both keys?</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      {Array.from({ length: value }).map((_, i) => (
        <span key={i} className={cn('rounded-full bg-amber-800/70', small ? 'h-1.5 w-1.5' : 'h-2 w-2 [@media(min-width:640px)_and_(min-height:560px)]:h-2.5 [@media(min-width:640px)_and_(min-height:560px)]:w-2.5')} />
      ))}
    </div>
  );
});
...
function PartRoundContent({ round, filled, chosenValue, wrongChoiceId, disabled }) {
  ...
  return (
    <div className="mt-1 flex w-full flex-1 flex-col items-center gap-2.5 [@media(min-width:640px)_and_(min-height:560px)]:mt-2 [@media(min-width:640px)_and_(min-height:560px)]:gap-4">
      <div className="flex flex-col items-center gap-1.5">
        <span className="font-body flex items-center gap-1.5 rounded-full bg-white/90 py-1 pl-3 pr-1.5 text-xs font-extrabold text-amber-800 shadow [@media(min-width:640px)_and_(min-height:560px)]:text-base">
      ...
      <div className="flex items-center justify-center gap-3 [@media(min-width:640px)_and_(min-height:560px)]:gap-5">
        <FixedKeySlot label="This key" value={fixedLabel} dotValue={round.fixedPart} />
        <span className="font-body rounded-full bg-amber-100 px-2.5 py-1 text-xs font-extrabold text-amber-700 [@media(min-width:640px)_and_(min-height:560px)]:text-sm">
          and
        </span>
...
const FixedKeySlot = React.memo(function FixedKeySlot({ label, value, dotValue }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      <span className="font-body text-[9px] font-extrabold uppercase tracking-wide text-amber-800/70 [@media(min-width:640px)_and_(min-height:560px)]:text-xs">
        {label}
      </span>
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl border-4 border-amber-700 bg-gradient-to-b from-amber-200 to-amber-400 shadow-[0_4px_0_rgba(0,0,0,0.2)] [@media(min-width:640px)_and_(min-height:560px)]:h-16 [@media(min-width:640px)_and_(min-height:560px)]:w-16">
        <span className="text-2xl [@media(min-width:640px)_and_(min-height:560px)]:text-3xl">🗝️</span>
      </div>
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <div
        ref={setNodeRef}
        className={cn(
          'flex h-14 w-14 items-center justify-center rounded-2xl border-4 border-dashed shadow-inner transition-colors [@media(min-width:640px)_and_(min-height:560px)]:h-16 [@media(min-width:640px)_and_(min-height:560px)]:w-16',
          filled ? 'border-emerald-500 bg-emerald-100' : isOver ? 'border-yellow-400 bg-white/70' : 'border-amber-500/60 bg-white/30'
        )}
      >
        <span className="text-2xl [@media(min-width:640px)_and_(min-height:560px)]:text-3xl">{filled ? '🗝️' : '❓'}</span>
      </div>
...
function KeyTray({ choices, format, disabled, wrongChoiceId }) {
  return (
    <div className="mt-1 flex w-full flex-wrap items-end justify-center gap-2.5 rounded-[1.5rem] border-4 border-dashed border-amber-400/60 bg-white/25 p-2.5 [@media(min-width:640px)_and_(min-height:560px)]:gap-4 [@media(min-width:640px)_and_(min-height:560px)]:p-4">
      {choices.map((c) => (
        <DraggableKey
          key={c.id}
          id={c.id}
...
const DraggableKey = React.memo(function DraggableKey({ id, value, dotValue, disabled, isWrong }) {
  ...
  return (
    <div
      ...
      className={cn(
        'flex flex-col items-center gap-1.5 rounded-2xl border-4 border-amber-700 bg-gradient-to-b from-yellow-100 to-amber-200 px-2.5 py-2.5 shadow-[0_5px_0_rgba(0,0,0,0.2)] transition-opacity [@media(min-width:640px)_and_(min-height:560px)]:px-3.5 [@media(min-width:640px)_and_(min-height:560px)]:py-3',
...
function WholeRoundContent({ round, chosenChestId, wrongChestId, disabled, onSelect }) {
  ...
  return (
    <div className="mt-1 flex w-full flex-1 flex-col items-center gap-3 [@media(min-width:640px)_and_(min-height:560px)]:mt-2 [@media(min-width:640px)_and_(min-height:560px)]:gap-5">
      <div className="flex items-center justify-center gap-2 rounded-[1.5rem] bg-white/90 px-4 py-3 shadow-[0_6px_0_rgba(0,0,0,0.15)] [@media(min-width:640px)_and_(min-height:560px)]:gap-3.5 [@media(min-width:640px)_and_(min-height:560px)]:px-6">
        ...
        <span className="text-xl text-amber-700/60 [@media(min-width:640px)_and_(min-height:560px)]:text-2xl">➡️</span>
        <span className="text-3xl [@media(min-width:640px)_and_(min-height:560px)]:text-4xl">❓</span>
      </div>

      <p className="font-body text-[11px] font-bold text-amber-900/80 [@media(min-width:640px)_and_(min-height:560px)]:text-sm">Tap the chest with the right number!</p>

      <div className="flex flex-wrap items-start justify-center gap-3 [@media(min-width:640px)_and_(min-height:560px)]:gap-6">
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/30 p-4">
      ...
      <motion.div
        ...
        className="relative flex max-w-sm flex-col items-center rounded-[2.5rem] bg-white px-8 py-6 text-center shadow-2xl [@media(min-width:640px)_and_(min-height:560px)]:py-8"
      >
        <img src="/chest_open.png" alt="Open treasure chest" className="w-24 select-none drop-shadow-lg [@media(min-width:640px)_and_(min-height:560px)]:w-28" draggable={false} />
        <p className="font-heading mt-1 text-2xl font-bold text-amber-500 [@media(min-width:640px)_and_(min-height:560px)]:text-3xl">
  ...
  return (
    <div className="relative mt-3 flex flex-col items-center rounded-[2.5rem] bg-white/90 px-6 py-6 text-center shadow-2xl [@media(min-width:640px)_and_(min-height:560px)]:mt-6 [@media(min-width:640px)_and_(min-height:560px)]:px-14 [@media(min-width:640px)_and_(min-height:560px)]:py-10">
      <Confetti
        ...
        style={{ position: 'fixed', inset: 0, zIndex: 20, pointerEvents: 'none' }}
      />
      <img src="/chest_open.png" alt="Open treasure chest" className="w-20 select-none drop-shadow-lg [@media(min-width:640px)_and_(min-height:560px)]:w-28" draggable={false} />
